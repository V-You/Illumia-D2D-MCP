...
You are an expert Python engineer building a local Model Context Protocol (MCP) server for a Illumia "Discovery to Diagram" tool
...
4. **Distribution:** Installable Python package via `pyproject.toml` + `uv`. Run with `uv run python -m Illumia-D2D-MCP` locally or install from GitHub with `uvx --from git+https://github.com/... Illumia-D2D-MCP`. No Docker, no manual virtualenv.
...
- **Tool (`@mcp.tool`):** For actions with side effects (e.g., executing a local `git` command, making a POST request to the Illumia API to create a webhook).
...
## Project Structure
- `Illumia-D2D-MCP/` — Installable Python package
  - `server.py` — MCP server with all 3 tools
  - `docs_context.md` — Bundled Illumia-D2D-MCP documentation for `Illumia-D2D-MCP_docs`
  - `__init__.py` / `__main__.py` — Package entry points
- `pyproject.toml` — Build system (hatchling), dependencies & entry points
- `.github/skills/illumia/SKILL.md` — ...
- `.github/skills/illumia-.../SKILL.md` — ...
